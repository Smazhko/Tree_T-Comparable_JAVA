...
public class Tree <T_GENERIC extends Comparable<? super T_GENERIC>> {

    // TODO: 29.05.2023
    //  Добавить возможность использовать Tree для любых сравниваемых типов данных
    //  То есть нужно параметризовать класс T дженериком <T extends Comparable<? super T>>
    ...
    public void add(T_GENERIC value) {
        if (root == null) {
            root = new Node(value);
            return;
        }
    ...
    private void add(Node current, T_GENERIC value) {
    ...
    public boolean contains(T_GENERIC value) {
        return findNode(root, value) != null;
    }

    private Node findNode(Node current, T_GENERIC value) {
        if (current == null) {
            return null;
        }
    ...
    public void remove(T_GENERIC value) {
        root = removeNode(root, value);
    }

    // Метод, который удаляет ноду и возвращает ту ноду, которая будет вместо удаленной
    private Node removeNode(Node current, T_GENERIC value) {
        if (current == null) {
            return null;
        }
    ...
    public List<T_GENERIC> dfs() {
        if (root == null) {
            return List.of();
        }

        List<T_GENERIC> result = new ArrayList<>();
        dfs(root, result);
        return List.copyOf(result);
    }

    private void dfs(Node current, List<T_GENERIC> result) {
        // in-order
        if (current.left != null) {
